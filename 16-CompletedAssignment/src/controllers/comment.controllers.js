...
const addComment = asyncHandler(async (req, res) => {
    ...
    const newComment=await Comment.create({
        videoId,content,owner:ref.user?._id
    });

    if(!newComment){
        throw new ApiError(400,"There was a problem while creating a comment");
    }
...
const updateComment = asyncHandler(async (req, res) => {
    ...
    if(!video){
        throw new ApiError("Video was not found");
    }
    ...
    if(!newComment){
        throw new ApiError(400,"There was issue while updating comment");
    }
...
const deleteComment = asyncHandler(async (req, res) => {
    ...
    if(!commentToBeDeleted){
        throw new ApiError(400,"There was some error while deleting a comment");
    }

    return res
    .status(200)
    .json(
        new ApiResponse(200,commentToBeDeleted,"Comment deleted successfully")
    );
